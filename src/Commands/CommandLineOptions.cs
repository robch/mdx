...
class CommandLineOptions
{
    ...
    public CommandLineOptions()
    {
        ...
        AllOptions = null;
        SaveAliasName = null;
        RepeatCount = 0; // 0 means no repetition (execute once)
    }
    ...
    private static bool TryParseGlobalCommandLineOptions(CommandLineOptions commandLineOptions, string[] args, ref int i, string arg)
    {
        ...
        else if (arg == "--repeat")
        {
            ...
            if (!int.TryParse(repeatStr, out int repeatCount) || repeatCount < 0)
            {
                throw new CommandLineException("Invalid value for --repeat. Must be a non-negative integer.");
            }
